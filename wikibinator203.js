...
from below... "TODO write my own opensource license"... has alot of useful info in that 1 page quote.
...
const wikibinator203 = (()=>{
	...
	*/
	...
	vm.Node = function(vm,l,r){
		...
		let isNowGettingIts7thParam = leftOpLessThan128 && lcur==1;
		...
		if(isLeaf){
			...
			if(isLessThan7Params){
				...
				if(op == vm.o8Of_opOneMoreParam){ //the only vararg op
					...
					if(newNumCurries  < vm.maxCurries){
						curriesLeft = newNumCurries;
					}else{
						throw 'TODO';
					}
		...
		TODO set curriesLeft and o8 and make header from it. the above code doesnt always set those. header also contains 6+2 bits for literal cbt256.
		TODO set bize
		...
		*/
		...
		if(isLeaf){
			FIXME REMOVE: this.curriesLeft = 7;
		}else if(lcur == 1){
	...
	vm.rootEvaler = (vm,l,r)=>{
		...
		if(l().curriesLeft > 1){
			...
			switch(o8){
				...
				break;case o.stackIsAllowMutableWrapperLambda:
					ret = vm.bit(vm.stackIsAllowMutableWrapperLambda);
				break;case o.s:
					ret = x(z)(y(z));
				break;case o.t:
					ret = y;
